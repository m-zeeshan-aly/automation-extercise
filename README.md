...
> Important test naming note: pytest only discovers functions that start with `test_`. If you see a function in the code that starts with `test_`, that test is intentionally disabled. Remove the leading `x` so it becomes `test_...`, then run the normal pytest command and the test will be executed.
...
- Opens signup/login through `get_button('Signup / Login')`
